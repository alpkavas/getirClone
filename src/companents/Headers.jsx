import { BiGlobe } from "react-icons/bi";
import { FaUser, FaUserPlus } from "react-icons/fa";

import React from "react";

function Headers() {
  return (
    <div>
      <nav>
        <div className="bg-header fixed top-0 left-0 right-0 z-30  ">
          <div className="container hover:text-opacity-100  mx-auto h-11 flex items-center justify-between  ">
            <div className="flex flex-row text-center items-center">
              <a
                href="#"
                className="  active:bg-primary hover:bg-primary pl-4 pr-4 pt-2 pb-4 mt-1.5 rounded-md ">
                <img
                  className="w-8 h-4"
                  data-testid="main-image"
                  src="data:image/svg+xml;base64,"
                />
              </a>
              <a
                href="#"
                className=" p-2 mb-2 font-Hind group  text-white text-opacity-80 hover:text-opacity-100 transition-all text-md  active:bg-primary hover:bg-primary  mt-1 rounded-md ">
                <span className="group-hover:text-brand-yellow">getir</span>
                yemek
              </a>
              <a
                href="#"
                className=" p-2 mb-2 font-Hind group  text-white text-opacity-80 hover:text-opacity-100 transition-all text-md  active:bg-primary hover:bg-primary  mt-1 rounded-md ">
                <span className="group-hover:text-brand-yellow">getir</span>
                büyük
              </a>
              <a
                href="#"
                className=" p-2 mb-2 font-Hind group text-white text-opacity-80 hover:text-opacity-100 transition-all text-md  active:bg-primary hover:bg-primary  mt-1 rounded-md ">
                <span className="group-hover:text-brand-yellow">getir</span>su
              </a>
              <a
                href="#"
                className=" p-2 mb-2 font-Hind group  text-white text-opacity-80 hover:text-opacity-100 transition-all text-md  active:bg-primary hover:bg-primary  mt-1 rounded-md ">
                <span className="group-hover:text-brand-yellow">getir</span>
                çarşı
              </a>
            </div>
            <nav className="flex flex-row gap-x-5 text-sm font-semibold">
              <a
                className="text-white text-opacity-80 hover:text-opacity-100 transition-all flex items-center"
                href="">
                <BiGlobe className="mr-1 mt-0.5 w-4 h-4" />
                Türkçe (TR)
              </a>
              <a
                className="text-white text-opacity-80  hover:text-opacity-100 transition-all flex items-center"
                href="">
                <FaUser className="mr-1" />
                Giriş Yap
              </a>
              <a
                className="text-white text-opacity-80 hover:text-opacity-100 transition-all flex items-center"
                href="">
                <FaUserPlus className="mr-1 w-4 h-4" />
                Kayıt Ol
              </a>
            </nav>
          </div>
        </div>
      </nav>
    </div>
  );
}

export default Headers;
